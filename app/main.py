...
async def register(
    ...
    otp_expires = datetime.now() + timedelta(minutes=int(auth.OTP_EXPIRE_MINUTES))
...
async def verify_otp(
    ...
    if user.otp_expires_at and datetime.now() > user.otp_expires_at.astimezone():
...
async def send_password_reset_otp(  # 🔄 changed from send_otp
    ...
    user.otp_expires_at = datetime.now() + timedelta(minutes=auth.OTP_EXPIRE_FOR_RESET_MINUTES)
    await db.commit()
